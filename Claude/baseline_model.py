"""
Baseline Model — Bank Marketing Term-Deposit Prediction
========================================================
...
job_yes_rate = df.groupby("job")["y_binary"].mean()
...
target = "y_binary"
drop_cols = [target]  # not a feature
...
# ═══════════════════════════════════════════════════════════════════════════
# 7.  SUMMARY TABLE  (CV + holdout, both models)
# ═══════════════════════════════════════════════════════════════════════════
print("\n" + "=" * 72)
...
results_df.to_csv(OUT_DIR / "baseline_results_summary.csv", index=False)

# ═══════════════════════════════════════════════════════════════════════════
# 8.  FIGURES: ROC curve + Precision-Recall curve (holdout)
# ═══════════════════════════════════════════════════════════════════════════
...
ax.set_title("Precision-Recall Curve — Holdout Set")
ax.legend(loc="upper right")

plt.tight_layout()
plt.savefig(OUT_DIR / "fig_baseline_roc_pr.png", dpi=150)
plt.close()
print("\nFigure saved: fig_baseline_roc_pr.png")
...
colors = ["#d62728" if v < 0 else "#2ca02c" for v in top20["coef"]]
ax.barh(top20["feature"], top20["coef"], color=colors, edgecolor="black")
ax.set_xlabel("Logistic Regression Coefficient")
ax.set_title("Top-20 Features by |coefficient| (Logistic Regression baseline)")
ax.axvline(0, color="black", linewidth=0.8)
plt.tight_layout()
plt.savefig(OUT_DIR / "fig_baseline_lr_coefficients.png", dpi=150)
plt.close()
print("\nFigure saved: fig_baseline_lr_coefficients.png")
...
print("  [OK] Preprocessing fitted on training split only (via Pipeline)")
...
print("  [OK] No target encoding or feature selection using full data")
print("  [OK] 'pdays' retained as numeric + binary flag (no target info used)")

# Verify no overlap between train and test index
