...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="Columns to drop before modelling.",
    )
    ...
    job_yes_rate = working_df.groupby("job")["y"].apply(lambda s: (s == "yes").mean())
    working_df["job_yes_rate"] = working_df["job"].map(job_yes_rate)
    # recompute after adding the new feature
    numeric_cols, categorical_cols = infer_feature_types(working_df, args.target)
    ...
    log_score = logistic.predict_proba(X_test)[:, 1]
